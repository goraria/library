...
class App(CTk):
    ...
    def RtnBook(self):
        ...
        if (name_book and id_person) != '':
            ...
            if checked != 0:
                date = "SELECT `borrowed_date` FROM `ReturnedForm` WHERE `id_person` = %s AND `id_book` = %s AND `returned_date` = '1001-01-01'"
                late = "SELECT `returned_date` FROM `ReturnedForm` WHERE `id_person` = %s AND `id_book` = %s AND `returned_date` > `deadline` IN (SELECT `daedline` FROM `BorrowedForm` WHERE `id_person` = %s AND `id_book` = %s"
                if myCursor.execute(date, (id_person, id_book)) != 0:
                    ...
                    try:
                        ...
                        myConnect.commit()
                        if myCursor.execute(late, (id_person, id_book)) != 0:
    ...
    def MemberInfo(self, value):  # value # evt
        ...
        index = 0
        for bor in checked:
            self.listInfo.insert(bor, str(dated[index][0]) + '-' + checked[index][0])
            count += 1
            index += 1
    ...
    def LentInfo(self, value):
        id_person = self.usernameSigninEntry.get().strip()
        query = "SELECT `borrowed_date`, `returned_date` FROM `ReturnedForm` WHERE id_person = %s AND `id_book` IN (SELCT `id_book` FROM `Book` WHERE `name_book` = %s)"
        myCursor.execute(query, (id_person, value.split('. ')[0]))
        lentInfo = myCursor.fetchall()
