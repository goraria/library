...
class App(CTk):
    def __init__(self):
        ...
        self.idSignupEntry = CTkEntry(self.signupFrame, width=240, placeholder_text="id person")
        self.idSignupEntry.grid(row=2, column=0, padx=30, pady=(15, 15))
        self.signupFormButton = CTkButton(self.signupFrame, text="Sign up", command=self.CheckID, width=240)
...
class Ware(CTk):
            ...
            if checked != 0:
                ...
                if myCursor.execute(date, (id_person, id_book)) != 0:
                    ...
                    try:
                        ...
                        if myCursor.execute(late, (id_person, id_book, str(dated[0][0]), str(quered[0][0]))) != 0:
                            messagebox.showinfo('Thành công', 'Trả sách muộn. Vui lòng trả trả VNĐ200.000,00 cho vé phạt!', icon='info')
                        else:
    ...
    def BookInfo(self, value):
        ...
        self.listDetails.insert(5, 'Lưu trữ: ' + bookInfo[0][6])
        ...
        quantity = int(bookInfo[0][5]) - int(checked[0][0])
        if (bookInfo[0][5] and (quantity)) != 0:
            self.listDetails.insert(6, 'Số lượng: khả dụng ' + str(quantity))
        else:
            self.listDetails.insert(6, 'Số lượng: không khả dụng')
    ...
    def MemberInfo(self, value):
        ...
        self.listInfo.delete('all', 'end')
        self.listInfo.insert(0, 'Họ Tên: ' + memberInfo[0][1])
        self.listInfo.insert(1, 'Giới tính: ' + memberInfo[0][2])
        self.listInfo.insert(2, 'Ngày sinh: ' + str(memberInfo[0][3]))
        self.listInfo.insert(3, 'Địa chỉ: ' + memberInfo[0][4])
        self.listInfo.insert(4, 'Đang mượn:')
    ...
    def DisplayBorrowAndReturn(self, value):
        ...
        for index in check:
            if str(index[4]) == '1001-01-01' or str(index[4]) == str(index[3]):
                self.listBorRtn.insert(count, str(index[0]) + '. Mượn: ' + '\n' + 'Mã sách: ' + str(index[2]) + '\n' + 'Mã độc giả: ' + str(index[1]))
                count += 1
            if str(index[4]) != '1001-01-01':
                self.listBorRtn.insert(count, str(index[0]) + '. Trả:  ' + '\n' + 'Mã sách: ' + str(index[2]) + '\n' + 'Mã độc giả: ' + str(index[1]))
                count += 1
    ...
    def FormInfos(self, value):
        ...
        myCursor.execute(deadline, (values[0][0:-8]))
        deadlined = myCursor.fetchall()
        myCursor.execute(query, (values[0][0:-8]))
        quered = myCursor.fetchall()
        myCursor.execute(person, values[2][12:])
        name_person = myCursor.fetchall()
        myCursor.execute(book, values[1][9:])
        name_book = myCursor.fetchall()

        self.listForm.delete('all', 'end')

        self.listForm.insert(0, values[0][0:])
        self.listForm.insert(1, 'Người mượn: ' + str(name_person[0][0]))
        self.listForm.insert(2, 'Tên sách: ' + str(name_book[0][0]))
        self.listForm.insert(3, 'Ngày mượn: ' + str(quered[0][3]))
        if str(quered[0][4]) != '1001-01-01':
            self.listForm.insert((4), 'Ngày trả: ' + str(quered[0][4]))
        else:
            self.listForm.insert((4), 'Hạn trả: ' + str(deadlined[0][0]))
    ...
    def LentInfo(self, value):
        ...
        for lent in lentInfo:
            ...
            self.listLentInfo.insert(count, 'Ngày mượn: ' + str(lent[0]))
            if str(lent[1]) != '1001-01-01':
                self.listLentInfo.insert((count + 1), 'Ngày trả: ' + str(lent[1]))
            else:
                self.listLentInfo.insert((count + 1), 'Hạn trả: ' + str(checked[0][0]))
    ...
    def AdminUI(self):
        ...
        ##
        self.tab2 = self.tabs.add('Độc giả')
        self.tab2.columnconfigure(0, weight=6)
        self.tab2.columnconfigure(1, weight=4)
        self.tab2.rowconfigure(0, weight=1)
        self.listMembers = CTkListbox(
        ...
        self.listInfo.grid(row=0, column=1, padx=(10, 10), pady=(0, 10), sticky='nsew')
        ##
        self.tab3 = self.tabs.add('Mượn & Trả')
        self.tab3.columnconfigure(0, weight=3)
        self.tab3.columnconfigure(1, weight=3)
        self.tab3.columnconfigure(2, weight=4)
        self.tab3.rowconfigure(0, weight=1)
        ...
        self.tab4 = self.tabs.add('Thống kê')
        self.labelBookCount = CTkLabel(self.tab4, text='', font=CTkFont('Arial', size=20, weight='bold'))
        self.labelBookCount.grid(row=0, padx=50, pady=(50, 20), sticky='w')
        self.labelMemberCount = CTkLabel(self.tab4, text='', font=CTkFont('Arial', size=20, weight='bold'))
        self.labelMemberCount.grid(row=1, padx=50, pady=20, sticky='w')
    ...
    def UserUI(self):
        ...
        self.tab5 = self.tabs.add('Sách mượn')
        self.tab5.columnconfigure(0, weight=6)
        self.tab5.columnconfigure(1, weight=4)
        self.tab5.rowconfigure(0, weight=1)
        self.listLenting = CTkListbox(
    ...
    def LogedOut(self):
        ...
        try:
            self.tabs.delete('Độc giả')
            self.tabs.delete('Mượn & Trả')
            self.tabs.delete('Thống kê')
        except:
            self.tabs.delete('Sách mượn')
